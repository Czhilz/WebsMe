...
function initDashboard(user) {
    ...
    if (welcomeNameEl)
        welcomeNameEl.textContent = fullname;
    if (userRoleEl)
        userRoleEl.textContent = role;
    if (welcomeRoleEl)
        welcomeRoleEl.textContent = role;
    updateThemeUI();
    document
...
async function loadStudentGrades(userId) {
    ...
    if (avgGradeEl)
        avgGradeEl.textContent = avgGrade.toString();
    if (totalSubjEl)
        totalSubjEl.textContent = grades.length.toString();
    if (statusEl) {
...
document
    ...
    const uh = parseInt(document.getElementById("inputUH").value, 10);
    const uts = parseInt(document.getElementById("inputUTS").value, 10);
    const uas = parseInt(document.getElementById("inputUAS").value, 10);
    if (!user_id) {
